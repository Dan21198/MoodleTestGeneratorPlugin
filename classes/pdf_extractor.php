...
class pdf_extractor {
    ...
    public function __construct() {
        $this->maxsize = get_config('local_quizgen', 'max_pdf_size') ?: 50;
        $this->maxlength = get_config('local_quizgen', 'max_text_length') ?: 15000;
    }
    ...
    public function extract_from_storedfile(\stored_file $file) {
        ...
        if ($filesize_mb > $this->maxsize) {
            return [
                'success' => false,
                'text' => '',
                'error' => get_string('error_file_too_large', 'local_quizgen', $this->maxsize)
            ];
        }
        ...
        if ($file->get_mimetype() !== 'application/pdf') {
            return [
                'success' => false,
                'text' => '',
                'error' => get_string('error_invalid_mimetype', 'local_quizgen')
            ];
        }
        ...
        if (empty($content)) {
            return [
                'success' => false,
                'text' => '',
                'error' => get_string('error_empty_file', 'local_quizgen')
            ];
        }
    ...
    public function extract_from_content($content, $filename = 'document.pdf') {
        ...
        if (!$this->is_valid_extracted_text($text)) {
            return [
                'success' => false,
                'text' => '',
                'error' => get_string('error_no_text_extracted', 'local_quizgen')
            ];
        }
    ...
    private function extract_with_pdftotext($content) {
        ...
        try {
            $tempdir = make_temp_directory('quizgen');
