...
class job_manager {
    ...
    public function create_job($fileid, $filename, $questioncount, $questiontype) {
        ...
        $jobid = $DB->insert_record('local_quizgen_jobs', $job);

        // Log the action
        \local_quizgen_log($this->courseid, $this->userid, 'job_created', $jobid, "File: $filename");
    ...
    public function create_job_multi($fileids, $filename, $questioncount, $questiontype) {
        ...
        \local_quizgen_log($this->courseid, $this->userid, 'job_created', $jobid, "Files: $filecount, Name: $filename");
    ...
    public function process_job($jobid) {
        ...
        $job = $DB->get_record('local_quizgen_jobs', ['id' => $jobid]);
        if (!$job) {
        ...
        try {
            ...
            \local_quizgen_log($this->courseid, $this->userid, 'processing_files', $jobid,
            ...
            \local_quizgen_log($this->courseid, $this->userid, 'question_distribution', $jobid, $distlog);
            ...
            foreach ($filedata as $index => $data) {
                ...
                if ($questioncount <= 0) {
                    \local_quizgen_log($this->courseid, $this->userid, 'file_skipped', $jobid,
                        "Skipping {$data['filename']} - 0 questions allocated");
                    continue;
                }

                \local_quizgen_log($this->courseid, $this->userid, 'generating_questions', $jobid,
                ...
                if (!empty($generation['raw_response'])) {
                    $existingResponse = $DB->get_field('local_quizgen_jobs', 'api_response', ['id' => $jobid]);
                ...
                \local_quizgen_log($this->courseid, $this->userid, 'questions_generated', $jobid,
            ...
            // Step 3: Create quiz
            $quizname = get_string('generated_quiz_name', 'local_quizgen', [
    ...
    private function store_questions($jobid, $questions) {
        ...
        foreach ($questions as $question) {
            ...
            $DB->insert_record('local_quizgen_questions', $record);
        }
    }
    ...
    private function update_question_moodle_ids($jobid, $questioncount) {
        ...
        $storedquestions = $DB->get_records('local_quizgen_questions', ['jobid' => $jobid], 'id ASC');

        $i = 0;
        foreach ($storedquestions as $stored) {
            if (isset($slots[$i + 1])) {
                $stored->moodle_questionid = $slots[$i + 1]->questionid;
                $DB->update_record('local_quizgen_questions', $stored);
            }
            $i++;
        }
    }
    ...
    private function update_job_status($jobid, $status) {
        ...
        $DB->set_field('local_quizgen_jobs', 'status', $status, ['id' => $jobid]);
        $DB->set_field('local_quizgen_jobs', 'timemodified', time(), ['id' => $jobid]);
    }
    ...
    private function update_job_field($jobid, $field, $value) {
        ...
        $DB->set_field('local_quizgen_jobs', $field, $value, ['id' => $jobid]);
    }
    ...
    private function complete_job($jobid, $quizid) {
        ...
        $DB->set_field('local_quizgen_jobs', 'status', 'completed', ['id' => $jobid]);
        $DB->set_field('local_quizgen_jobs', 'quizid', $quizid, ['id' => $jobid]);
        $DB->set_field('local_quizgen_jobs', 'timecompleted', time(), ['id' => $jobid]);
        $DB->set_field('local_quizgen_jobs', 'timemodified', time(), ['id' => $jobid]);

        \local_quizgen_log($this->courseid, $this->userid, 'job_completed', $jobid, "Quiz ID: $quizid");
    }
    ...
    private function fail_job($jobid, $error) {
        ...
        $DB->set_field('local_quizgen_jobs', 'status', 'failed', ['id' => $jobid]);
        $DB->set_field('local_quizgen_jobs', 'error_message', $error, ['id' => $jobid]);
        $DB->set_field('local_quizgen_jobs', 'timemodified', time(), ['id' => $jobid]);

        \local_quizgen_log($this->courseid, $this->userid, 'job_failed', $jobid, text_helper::truncate($error, 500));
    }
    ...
    public function get_job($jobid) {
        global $DB;
        return $DB->get_record('local_quizgen_jobs', ['id' => $jobid]);
    }
    ...
    public function delete_job($jobid) {
        ...
        $DB->delete_records('local_quizgen_questions', ['jobid' => $jobid]);

        // Delete job
        $DB->delete_records('local_quizgen_jobs', ['id' => $jobid]);

        \local_quizgen_log($this->courseid, $this->userid, 'job_deleted', $jobid);
    ...
    public function retry_job($jobid) {
        ...
        $DB->set_field('local_quizgen_jobs', 'status', 'processing', ['id' => $jobid]);
        $DB->set_field('local_quizgen_jobs', 'error_message', null, ['id' => $jobid]);
        $DB->set_field('local_quizgen_jobs', 'timemodified', time(), ['id' => $jobid]);
