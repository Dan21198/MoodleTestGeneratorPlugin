...
class cleanup_old_data extends \core\task\scheduled_task {
    ...
    public function get_name() {
        return get_string('task_cleanup_old_data', 'local_quizgen');
    }
    ...
    public function execute() {
        ...
        // Delete old logs
        $logcount = $DB->count_records_select('local_quizgen_logs', 'timecreated < :cutoff', ['cutoff' => $cutoff]);
        if ($logcount > 0) {
        ...
        $orphancount = $DB->count_records_sql(
            "SELECT COUNT(*)
               FROM {local_quizgen_questions} q
               LEFT JOIN {local_quizgen_jobs} j ON q.jobid = j.id
              WHERE j.id IS NULL"
        );
        if ($orphancount > 0) {
            $DB->execute(
                "DELETE FROM {local_quizgen_questions}
                  WHERE jobid NOT IN (SELECT id FROM {local_quizgen_jobs})"
            );
        ...
        foreach ($oldjobs as $job) {
            ...
            // Delete questions
            $DB->delete_records('local_quizgen_questions', ['jobid' => $job->id]);

            // Delete job
            $DB->delete_records('local_quizgen_jobs', ['id' => $job->id]);
            $deletedjobs++;
        }
