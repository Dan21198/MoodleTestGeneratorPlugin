...
use local_quizgen\question\question_helper;
use local_quizgen\question\question_type_factory;
class quiz_generator {
    ...
    public function create_quiz(array $questions, string $quizname, array $quizoptions = []): array {
        ...
        try {
            ...
            if (!$categoryid) {
                throw new \moodle_exception('error_category_creation', 'local_quizgen');
            }

            $quiz = $this->create_quiz_activity($quizname, $quizoptions);
            if (!$quiz) {
                throw new \moodle_exception('error_quiz_creation', 'local_quizgen');
            }
            ...
            if (empty($result['questionids'])) {
                ...
                throw new \moodle_exception('error_no_questions_created', 'local_quizgen', '', null, $errormsg);
            }
            ...
            if ($slotcount == 0) {
                $this->delete_quiz($quiz->id);
                throw new \moodle_exception('error_no_slots', 'local_quizgen', '', null,
    ...
    private function validate_questions(array $questions): void {
        if (empty($questions)) {
            throw new \moodle_exception('error_no_questions_provided', 'local_quizgen', '', null,
    ...
    private function get_or_create_question_category(): int {
        ...
        $category->info = 'Questions generated by MoodleTestGeneratorPlugin';
    ...
    private function build_quiz_record(string $name, array $options): object {
        $quizintro = $options['intro'] ?? get_string('default_quiz_intro', 'local_quizgen');
